...
def _parametri(
    uscita_mq: float, mq: float, libero: bool, costo_mq: float
) -> Parametri:
    ...
    return Parametri(
        ...
        mesi_totali=MESI_LIBERO if libero else MESI_OCCUPATO,
...
def soglia_screening(
    ...
    """
    p = _parametri(uscita_mq, mq, libero, costo_mq)
...
def tabella(
    ...
    for t in obiettivi:
        for libero in (False, True):
            r = soglia_screening(t, uscita_mq, mq, libero, costo_mq)
            r["libero"] = 1.0 if libero else 0.0
            righe.append(r)
    return righe


def _stampa_tabella(uscita_mq: float, mq: float, costo_mq: float) -> None:
    ...
    print("-" * 78)
    righe = tabella(uscita_mq, mq, costo_mq)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if a.roi is None:
        _stampa_tabella(a.uscita_mq, a.mq, a.costo_mq)
    else:
        r = soglia_screening(a.roi, a.uscita_mq, a.mq, a.libero, a.costo_mq)
